...
package com.mardous.geet.helper.color;
...
import com.mardous.geet.extensions.resources.ColorExtKt;
...
public class MediaNotificationProcessor {
    ...
    private boolean isRecycled(Drawable drawable) {
        if (drawable instanceof BitmapDrawable) {
            Bitmap wrappedBitmap = ((BitmapDrawable) drawable).getBitmap();
    ...
    public int getMightyColor() {
        boolean isDarkBg =
                ColorExtKt.isColorLight(ColorExtKt.surfaceColor(context));
        if (isDarkBg) {
            if (isColorLight(backgroundColor)) {
                return primaryTextColor;
            } else {
